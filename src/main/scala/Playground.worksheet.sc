...
object BookRepository1 {
  ...
  def doobieInterpreter[F[_]](transactor: Transactor[F])(implicit m: MonadCancel[F,Throwable]): BookRepository1[F] = new BookRepository1[F] {
    def getAllBooks: F[List[Book]] =
    ...
    def addBook(book: Book): F[Unit] =
      sql"INSERT INTO books (title, author) VALUES (${book.title}, ${book.author})".update.run.transact(transactor).void

    def updateBook(book: Book): F[Unit] =
      sql"UPDATE books SET title = ${book.title}, author = ${book.author} WHERE id = ${book.id}".update.run.transact(transactor).void
...
object UserRepository {
  ...
  def doobieInterpreter[F[_]](transactor: Transactor[F])(implicit m:MonadCancel[F,Throwable]): UserRepository[F] = new UserRepository[F] {
    // existing code...
...
def getUserById(userId: Long): F[Option[User]]= m.pure(None)
    ...
    def updateUser(user: User, roles: List[Role]): F[Unit] =
      for {
        _ <- sql"UPDATE users SET username = ${user.userName}, password_hash = ${user.passwordHash}, email = ${user.email}, is_active = ${user.isActive} WHERE id = ${user.id}".update.run.transact(transactor).void
    ...
    private def addRolesToUser(userId: Long, roles: List[Role]): F[Unit] =
      roles.traverse_ { role =>
        sql"INSERT INTO user_roles (user_id, role_id) VALUES ($userId, ${role.id})".update.run.transact(transactor).void
      }
    ...
    def getUserRoles(userId: Long): F[List[Role]] =
      sql"SELECT r.id, r.role_name, r.description FROM roles r INNER JOIN user_roles ur ON r.id = ur.role_id WHERE ur.user_id = $userId".query[Role].to[List].transact(transactor)
  }
