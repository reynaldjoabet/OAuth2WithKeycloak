...
final case class AuthenticationRoutes[F[_]: Async: Console](
  ...
  val routes: HttpRoutes[F] = HttpRoutes.of[F] {
    case request @ GET -> Root / "login" :? FrontendRedirectParam(redirect) =>
      getFrontendRedirectUrl(redirect)
        .product(isValidSession(request))
      ...
      code -> state match {
        ...
        case (Some(code), Some(state)) =>
          (for {
            ...
            userInfo <- clientService.getUserInfo(tokenEndpointResponse.accessToken)
            // .flatTap(Console[F].println)
  ...
  val routes2 = AuthedRoutes.of[UserSession, F] {
    ...
    case request @ POST -> Root / "logout" as userSession =>
      ...
      } yield ()).flatMap(_ => Ok()).map(_.removeCookie(COOKIE_NAME)).uncancelable // By default the logout endpoint will redirect back to the root of the application after logout is done.
    ...
    case req @ GET -> Root / "user" as userSession =>
      ...
      (for {
        userInfo <- clientService.getUserInfo(userSession.accessToken)
    ...
    case req @ POST -> Root / "backchannel" / "logout" as userSession =>
      ...
      req
        .req
        .as[BackChannelLogoutRequest]
        .flatMap { backChannelLogoutRequest =>
          for {
  ...
  def getFrontendUrlFromState(state: String): F[Option[String]] =
    tokenService.getState(state).<*(tokenService.deleteState(state).void)
  ...
  /**
    * Int has has a kind of A, String has a kind of A. Kleisli[F[_],A,B] this is a type constructor
    * that takes a type constructor(s). A higher -kinder type Types like List that take one argument
    * are called type constructors. List is a type constructor, a first order kinded type, Function1
    * like Either and Map are binary type constructors( takes two aruguments) Function1[A,B],
    * Either[A,B] and Map[A,B] are proper types. A=>? is a type constructor that when you apply a
    * proper type, say X, you get back a function A=>X . F[_] is a type constructor and F[A] is a
    * proper type .Higher-kinded types are type constructors that take other types( or even other
    * type constructors) as parameter
    *
    * trait Functor[F[_]] expects a type constructor with one parameter Functor[List],
    * Functor[Option] but not Functor[Map] as Map takes 2 parameters(Map[K,V]) while the type
    * parameter to Functor experts one Partial applications of type constructors We can use type
    * aliases to partially apply a type constructor and so adapt the kind of the type to be used
    * type IntkeyMap[A]=Map[Int,A] Functor[IntKeyMap]// works now. { type T[Y] = OptionT[F, Y] }
    * defines a structural type denoted by {} with a type alias inside. { type T[Y] = OptionT[F, Y]
    * })#T defines an anonymous type, inside of which is defined a type alia and then accessing the
    * type alias with the # syntax # is type projection, used to reference the type member T of the
    * structural type { type T[Y] = OptionT[F, Y] })#T is a type lamda
    * @return
    */
